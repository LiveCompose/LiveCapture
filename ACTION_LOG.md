...
2025-09-24
- 根据 NEXT_MOVE.md 执行两项关键修复：
  - 坐标系转换：在 `ContentView.swift` 中将 Vision 左下原点的归一化矩形先转换为 MetadataOutput 左上原点坐标（翻转 Y），再通过 `layerRectConverted(fromMetadataOutputRect:)` 映射到 `AVCaptureVideoPreviewLayer` 像素坐标；修复跟踪框偏差。
  - 跟踪延迟优化：在 `TrackingManager.swift` 将平滑窗口从 5 降为 3，在保持稳定性的同时降低显示延迟。
- 影响：跟踪框与画面对齐更准确，目标不再“拖尾”。

- 2025-09-24: 新增模板匹配追踪方法
  - 新增 `TemplateMatcher.swift`：
    - 从模型返回的检测框内裁取中心正方形作为模板（灰度、归一化、64×64）。
